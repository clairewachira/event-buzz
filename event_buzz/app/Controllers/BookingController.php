...
class BookingController extends BaseController
{
    ...
    public function bookings()
    {
        $data = [
            ...
            'bookings' => $this->bookingModel->findBookingsByUser(session()->get('user_id')),
        ];

        $data['content'] = view('user/bookings', $data);
	...
	public function store()
	{
		...
		$data["amountPaid"] = $data["numberOfTickets"] * $data['price'];
		$data['userId'] = session()->get('user_id');
		$event = $this->eventModel->find($data['eventId']);
		$remaining = $event['availableTickets'] - $data['numberOfTickets'];
		$this->eventModel->updateEvent($data['eventId'], ["availableTickets" => $remaining]);
